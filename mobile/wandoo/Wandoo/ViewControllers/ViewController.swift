...
//    }
    ...
    func retrieveWandoos() {
        ...
        if String(self.view.subviews.last).containsString("UIImageView") {
            dispatch_async(dispatch_get_main_queue()) {
        ...
        getAllWandoos { (allWandoos) -> Void in
            ...
            self.userModel.getUserInfo(fbID, completion: { (userInfo) -> Void in
                ...
                dispatch_async(dispatch_get_main_queue()){
                    ...
                    if self.allWandoosArray.count == 0 {
                        let noWandooImage = UIImage(named: "noWandoo")
                        let noWandoos = UIImageView(image: noWandooImage)
                        noWandoos.frame = CGRect(x: self.view.bounds.width/2 - 150, y: self.view.bounds.height/2 - 230, width: 300, height: 500)
